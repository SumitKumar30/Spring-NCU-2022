...
public class MyDemoLoggingAspect {
	...
	// Add advice for @Around on findAccounts method
	  @Around("execution(* org.ncu.spring_aop_demo.dao.AccountDaoImpl.findAccounts(..))"
	  ) public Object aroundAdvice(ProceedingJoinPoint proceedingPoint) throws
	  Throwable {
		  System.out.println("\n=======>>>>> Logging @Around findAccounts(..) : "
				  +proceedingPoint.getSignature().getName()+": Before method execution");
		  Object value = proceedingPoint.proceed();
		  System.out.println("\n=======>>>>> Logging @Around findAccounts(..) : "
				  +proceedingPoint.getSignature().getName()+": After method execution"); 
		 return value;
	  }
	...
	// Add advice for @After on findAccounts method
	@After("execution(* org.ncu.spring_aop_demo.dao.AccountDaoImpl.findAccounts(..))")
	public void afterFinallyAdvice(JoinPoint jPoint) {
	...
	public void afterReturningAdvice(JoinPoint jPoint, List<Account> result) {
	  ...
	  void beforeAddAccount(JoinPoint theJoinPoint) {
		  System.out.println("\n=====>> Executing @Before logging aspect"); 
